...
mod.s <- summary(lmer(scale(SR) ~ pf + (1|harvest/PSP), data = nf)) # standardized slope: -0.56
slopes$slope[1] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(SR) ~ pf + (1|harvest/PSP), data = nf))
slopes$upr[1] <- CI[5,2]
slopes$lwr[1] <- CI[5,1]
...
mod.s <- summary(lmer(scale(SR) ~ pf + (1|harvest), data = nf.psp)) # standardized slope: -0.56
slopes.psp$slope[1] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(SR) ~ pf + (1|harvest), data = nf.psp))
slopes.psp$upr[1] <- CI[4,2]
slopes.psp$lwr[1] <- CI[4,1]
...
mod <- lmer(SR ~ pf + (1|harvest/PSP), data = nf)
mod %>% summary
...
mod <- lmer(SR ~ pf + (1|harvest), data = nf.psp)
mod %>% summary
...
mod.s <- summary(lmer(scale(E) ~ pf + (1|harvest/PSP), data = nf)) # standardized slope: -0.56
slopes$slope[2] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(E) ~ pf + (1|harvest/PSP), data = nf))
slopes$upr[2] <- CI[5,2]
slopes$lwr[2] <- CI[5,1]
...
mod.s <- summary(lmer(scale(E) ~ pf + (1|harvest), data = nf.psp)) # standardized slope: -0.56
slopes.psp$slope[2] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(E) ~ pf + (1|harvest), data = nf.psp))
slopes.psp$upr[2] <- CI[4,2]
slopes.psp$lwr[2] <- CI[4,1]
...
mod <- lmer(E ~ pf +  (1|harvest/PSP), data = nf)
mod %>% summary
...
mod <- lmer(E ~ pf + (1|harvest), data = nf.psp)
mod %>% summary
...
mod.s <- summary(lmer(scale(WD) ~ pf + (1|harvest/PSP), data = nf)) # standardized slope: -0.56
slopes$slope[3] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(WD) ~ pf + (1|harvest/PSP), data = nf))
slopes$upr[3] <- CI[5,2]
slopes$lwr[3] <- CI[5,1]
...
mod.s <- summary(lmer(scale(WD) ~ pf + (1|harvest), data = nf.psp)) # standardized slope: -0.56
slopes.psp$slope[3] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(WD) ~ pf + (1|harvest), data = nf.psp))
slopes.psp$upr[3] <- CI[4,2]
slopes.psp$lwr[3] <- CI[4,1]
...
mod <- lmer(WD ~ pf +  (1|harvest/PSP), data = nf)
mod %>% summary # -0.05
...
mod <- lmer(WD ~ pf + (1|harvest), data = nf.psp)
mod %>% summary # -0.05
...
mod.s <- summary(lmer(scale(RGR) ~ pf + (1|harvest/PSP), data = nf)) # standardized slope: 0.56
slopes$slope[4] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(RGR) ~ pf + (1|harvest/PSP), data = nf))
slopes$upr[4] <- CI[5,2]
slopes$lwr[4] <- CI[5,1]
...
mod.s <- summary(lmer(scale(RGR) ~ pf + (1|harvest), data = nf.psp)) # standardized slope: 0.56
slopes.psp$slope[4] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(RGR) ~ pf + (1|harvest), data = nf.psp))
slopes.psp$upr[4] <- CI[4,2]
slopes.psp$lwr[4] <- CI[4,1]
...
mod <- lmer(RGR ~ pf + (1|harvest/PSP), data = nf)
mod %>% summary # 0.51
...
mod <- lmer(RGR ~ pf + (1|harvest), data = nf.psp)
mod %>% summary # 0.51
...
mod.s <- summary(lmer(scale(H) ~ pf + (1|harvest/PSP), data = H.df)) # standardized slope: -0.56
slopes$slope[5] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(H) ~ pf + (1|harvest/PSP), data = H.df))
slopes$upr[5] <- CI[5,2]
slopes$lwr[5] <- CI[5,1]
...
mod.s <- summary(lmer(scale(H) ~ pf + (1|harvest), data = H.df.psp)) # standardized slope: -0.56
slopes.psp$slope[5] <- mod.s$coefficients[2,1]


CI <- confint(lmer(scale(H) ~ pf + (1|harvest), data = H.df.psp))
slopes.psp$upr[5] <- CI[4,2] # modified
slopes.psp$lwr[5] <- CI[4,1] # modified
...
mod <- lmer(H ~ pf + (1|harvest/PSP), data = H.df)
summary(mod) #  sig
...
mod <- lmer(H ~ pf + (1|harvest), data = H.df.psp)
summary(mod) #  sig
...
h.nf.plot <- ggplot() +
     ...
     annotate('segment',
              ...
              size = 1.5, color = plot.color, alpha = 0.7) +
     geom_ribbon(data = H.df.psp, aes(x = ci.list7.psp$ci.df$x, ymin = ci.list7.psp$ci.df$lwr, ymax = ci.list7.psp$ci.df$upr),
                 alpha = 0.3, fill = 'grey30', color = plot.color) +
...
mod.s <- summary(lmer(scale(M) ~ pf + (1|harvest/PSP), data = nf)) # standardized slope: -0.56
slopes$slope[6] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(M) ~ pf + (1|harvest/PSP), data = nf))
slopes$upr[6] <- CI[5,2]
slopes$lwr[6] <- CI[5,1]
...
mod.s <- summary(lmer(scale(M) ~ pf + (1|harvest), data = nf.psp)) # standardized slope: -0.56

slopes.psp$slope[6] <- mod.s$coefficients[2,1]

CI <- confint(lmer(scale(M) ~ pf + (1|harvest), data = nf.psp))
slopes.psp$upr[6] <- CI[4,2]
slopes.psp$lwr[6] <- CI[4,1]
...
mod <- lmer(M ~ pf + (1|harvest/PSP), data = nf)
summary(mod)
...
mod <- lmer(M ~ pf + (1|harvest), data = nf.psp)
summary(mod)
...
figure(
     ggarrange(
          ...
          nrow = 4, ncol = 2, 
          align = 'hv'),
     path.name = 'c:/users/bryce/OneDrive/Documents/VMFR/Dynamics Paper 2019/Figures/Nfix_traits_nonfix_harvest',
     height = 12.1,
     width = 8,
     save = T,
     image.type = 'pdf'
...
figure(
     ggarrange(s1, s2,
               nrow = 2,
               align = 'hv'),
     path.name = 'c:/users/bryce/OneDrive/Documents/VMFR/Dynamics Paper 2019/Figures/Nfix_traits_nonfix_sidepanel_harvest',
     height = 3,
     width = 7.3,
     save = T
)
...
CI <- confint(lmer(scale(H) ~ pf + (1|harvest), data = H.df.psp))
slopes.psp$upr[5] <- CI[4,2]
slopes.psp$lwr[5] <- CI[4,2]
...
M.nf.plot <- ggplot() +
     ...
     theme(panel.background = element_blank(),
           ...
           axis.text = element_text(size = text.size),
           axis.title = element_text(size = text.size)) +
     labs(x = '', y = bquote('Mortality (%/year)'))
...
figure(
     ggarrange(SR.nf.plot,
               ...
               nrow = 3, ncol = 2, 
               align = 'hv'),
     path = 'c:/users/bryce/OneDrive/Documents/VMFR/Dynamics Paper 2019/Figures/Nfix_traits_fix_harvest',
     height = 9,
     width = 8,
     save = T
)
...
figure(
     ggarrange(s1, s2,
               nrow = 2,
               align = 'hv'),
     path = 'c:/users/bryce/OneDrive/Documents/VMFR/Dynamics Paper 2019/Figures/Nfix_traits_fix_sidepanel_harvest2',
     height = 3.3,
     width = 7.3,
     save = T
)
