...
mod <- lmer(M ~ Year + (1|PSP), data = M.sp)
mod %>% summary
...
mod <- lmer(I ~ Year + (1|PSP), data = I.sp)
mod %>% summary
...
# data
AGB.data <- read.csv('VMFR_corrected_AGB_diverse.csv') %>%
  dplyr::select(PSP, 16:37) %>% 
  dplyr::group_by(PSP) %>% 
  melt(id.vars = c('PSP'), value.name = 'AGB', variable.name = 'Year') %>%
  dplyr::mutate(Year = substr(Year,2,5) %>% as.numeric) %>%
  dplyr::filter(!is.na(AGB)) %>%
  dplyr::group_by(PSP, Year) %>% 
  dplyr::summarise(AGB = sum(AGB/1000)) %>%
  filter(AGB > 50) %>%
  mutate(time = Year - 1982) %>%
  filter(PSP %!in% bad.plots.25)

AGB.sp <- read.csv('VMFR_corrected_AGB_diverse.csv') %>%
  dplyr::select(PSP,Su.Pl.No, 16:37) %>% 
  dplyr::group_by(PSP, Su.Pl.No) %>% 
  melt(id.vars = c('PSP', 'Su.Pl.No'), value.name = 'AGB', variable.name = 'Year') %>%
  dplyr::mutate(Year = substr(Year,2,5) %>% as.numeric) %>%
  dplyr::filter(!is.na(AGB)) %>%
...
mod <- lmer(AGB ~ Year + (1|PSP), data = AGB.data)
mod %>% summary
...
mod <- lmer(AGB ~ Year + (1|PSP), data = AGB.sp)
mod %>% summary
...
mod <- lmer(BA ~ Year + (1|PSP), data = ba.data)
mod %>% summary
...
mod <- lmer(BA ~ Year + (1|PSP), data = ba.sp)
mod %>% summary
...
mod <- lmer(N.psp ~ Year + (1|PSP), data = d.data)
mod %>% summary
...
mod <- lmer(N.sp ~ Year + (1|PSP), data = d.sp)
mod %>% summary
